...
int main()
{
    ...
    for(int i = 0; i <= N; i++)    {
        z[i] = -2.0 + (4.0*i)/(N); } //guarda todos os valores que tanto a parte real quanto a imaginaria podem assume em z(0), isto é, -2 <= Rz <= 2 e -2 <= Iz <= 2.
    ...
    for(int re = 0; re <= N; re ++) {
        for(int im = 0; im <= N; im ++) {
            ...
            if (succ) {
                int verifica = 0;
                for(int i = 0; i < u; i++){
                    if (fabs(creal(r) - creal(raizes[i])) <= 1e-6 && fabs(cimag(r) - cimag(raizes[i])) <= 1e-6)
                        verifica = 1;           
                } 
                if (verifica == 0 && u < GRAU)
                {
                    raizes[u] = r;
                    u++;
                }
